...
export interface SeasonMatrixEntry {
  season: SeasonMode;
  primary_services: string[];
  blocked_services: string[];
  upgrade_bias: boolean;
  bias_direction?: "heating" | "cooling";
}

export interface Constitution {
  constitution_version: string;
  north_star: string;
  season_matrix: Record<string, SeasonMatrixEntry>;
  budget_enforcement: {
    max_weekly_change_pct: number;
    requires_override_artifact: boolean;
  };
  geo_controls: {
    core_zip_count: number;
    core_zip_min_weight: number;
    new_zip_activation_cap_per_week: number;
    default_weight: number;
  };
  capacity_protection: {
    weekly_lead_cap_default: number;
    conservative_trigger_pct: number;
    hard_throttle_trigger_pct: number;
  };
  invariants: string[];
}

export interface ConstitutionBoot {
...
export interface GatekeeperContext {
  month: number;
  week_budget: number;
  prior_week_budget: number;
  weekly_lead_projection: number;
  weekly_lead_cap: number;
  install_revenue_ratio: number | null;
  zip_weights: Record<string, number>;
  newly_activated_zips: string[];
  approvals: string[];
}

export type GatekeeperMode = "NORMAL" | "CONSERVATIVE" | "HARD_THROTTLE";
...
export interface GatekeeperResult {
  ...
  alerts: GatekeeperAlert[];
  season: SeasonMatrixEntry;
  constitution_version: string;
  constitution_hash: string;
  frozen_geo_expansion: boolean;
  frozen_budget_increase: boolean;
}

export interface GatekeeperAlert {
  type: "CONSERVATIVE_MODE" | "HARD_THROTTLE" | "BUDGET_VIOLATION" | "GEO_VIOLATION" | "SEASON_VIOLATION" | "DATA_MISSING";
  message: string;
  timestamp: string;
}
