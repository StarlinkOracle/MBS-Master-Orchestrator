 ...
 * Constitution-governed: FAIL CLOSED if constitution missing/invalid.
 */
...
import { bootConstitution } from "../core/constitution/loader.js";
import { evaluateGatekeeper, buildGatekeeperContext } from "../core/constitution/gatekeeper.js";
import type { GatekeeperResult, ConstitutionBoot } from "../types/index.js";
import { writeJSON, writeText, readJSON, ensureDir, currentWeekNumber, safeReadJSON, GENERATOR_VERSION } from "../utils/index.js";
import { join, resolve } from "path";
import { existsSync } from "fs";
import { getAllZipWeights } from "../core/geo/geoPriorityEngine.js";

// ---- Constitution Boot (FAIL CLOSED) ----
let constitutionBoot: ConstitutionBoot;
try {
  constitutionBoot = bootConstitution();
  console.log(`🏛️  Constitution v${constitutionBoot.constitution_version} loaded (hash: ${constitutionBoot.constitution_hash.slice(0, 16)}...)`);
} catch (err: any) {
  console.error(`\n❌ FATAL: ${err.message}\n`);
  process.exit(1);
}
...
// ---- Gatekeeper Helper ----
function runGatekeeper(overrides?: Partial<Parameters<typeof buildGatekeeperContext>[0]>): GatekeeperResult {
  const ROOT = process.cwd();
  const flowState = safeReadJSON<any>(resolve(ROOT, "config/flow_state.json"), {});
  const flowControl = safeReadJSON<any>(resolve(ROOT, "config/flow_control.json"), {});
  const adsConfig = safeReadJSON<any>(resolve(ROOT, "config/ads.json"), {});

  // Compute weekly lead projection from daily qualified leads * 5 workdays
  const dailyLeads = flowState.qualifiedLeadsToday || 0;
  const weeklyProjection = dailyLeads * 5;

  // Weekly lead cap from constitution default unless admin override
  const adminOverridePath = resolve(ROOT, "config/admin_override.json");
  const adminOverride = safeReadJSON<any>(adminOverridePath, null);
  const weeklyLeadCap = adminOverride?.weekly_lead_cap ??
    constitutionBoot.constitution.capacity_protection.weekly_lead_cap_default;

  // Install revenue ratio (null if unknown → conservative)
  const totalLeads = flowState.totalLeadsThisWeek || 0;
  const installLeads = flowState.installLeadsThisWeek || 0;
  const installRevenueRatio = totalLeads > 0 ? installLeads / totalLeads : null;

  const zipWeights = getAllZipWeights();

  // Budget from ads config or flow state
  const weekBudget = adsConfig?.google?.defaultDailyBudget ? adsConfig.google.defaultDailyBudget * 7 : 525;
  const priorWeekBudget = adsConfig?.google?.priorWeekDailyBudget ? adsConfig.google.priorWeekDailyBudget * 7 : weekBudget;

  // Approvals
  const approvalFiles: string[] = [];
  const approvalsDir = resolve(ROOT, "config/approvals");
  if (existsSync(approvalsDir)) {
    try {
      const files = require("fs").readdirSync(approvalsDir);
      approvalFiles.push(...files.map((f: string) => f.replace(/\.json$/, "")));
    } catch { /* ok */ }
  }

  const ctx = buildGatekeeperContext({
    weeklyLeadProjection: weeklyProjection,
    weeklyLeadCap,
    installRevenueRatio,
    zipWeights,
    newlyActivatedZips: [],
  ...
  return evaluateGatekeeper(ctx);
}

function printGatekeeperStatus(gk: GatekeeperResult): void {
  if (gk.mode !== "NORMAL") {
    console.log(`\n⚠️  Gatekeeper Mode: ${gk.mode}`);
  }
  if (gk.frozen_geo_expansion) console.log(`   🧊 Geo expansion FROZEN`);
  if (gk.frozen_budget_increase) console.log(`   🧊 Budget increase FROZEN`);
  for (const v of gk.violations) {
    const icon = v.severity === "block" ? "🛑" : "⚠️";
    console.log(`   ${icon} [${v.rule}] ${v.message}`);
  }
  for (const a of gk.alerts) {
...
switch (command) {
  ...
  case "plan": {
    ...
    // ---- Gatekeeper check ----
    const gk = runGatekeeper();
    printGatekeeperStatus(gk);

    if (gk.mode === "HARD_THROTTLE") {
      console.error(`\n🛑 HARD THROTTLE: Plan generation blocked by constitution gatekeeper.`);
      // Write alert
      const alertPath = join(process.cwd(), "bundles", "alerts");
      ensureDir(alertPath);
      writeJSON(join(alertPath, `hard-throttle-${new Date().toISOString().split("T")[0]}.json`), {
        type: "HARD_THROTTLE",
        timestamp: new Date().toISOString(),
        constitution_version: gk.constitution_version,
        constitution_hash: gk.constitution_hash,
        violations: gk.violations,
        alerts: gk.alerts,
      });
      process.exit(1);
    }
    ...
    if (result.status === "EXECUTED" && result.data) {
      ...
      // Embed constitution metadata
      (plan as any).constitution_version = gk.constitution_version;
      (plan as any).constitution_hash = gk.constitution_hash;
      (plan as any).gatekeeper_mode = gk.mode;
      ...
      console.log(`Gatekeeper: ${gk.mode} | Constitution: v${gk.constitution_version}`);
  ...
  case "bundle": {
    ...
    if (gk.mode === "HARD_THROTTLE") {
      console.error(`\n🛑 HARD THROTTLE: Bundle generation blocked by constitution gatekeeper.`);
      const alertPath = join(process.cwd(), "bundles", "alerts");
      ensureDir(alertPath);
      writeJSON(join(alertPath, `hard-throttle-bundle-${new Date().toISOString().split("T")[0]}.json`), {
        type: "HARD_THROTTLE",
        timestamp: new Date().toISOString(),
        constitution_version: gk.constitution_version,
        constitution_hash: gk.constitution_hash,
    ...
    if (result.status === "EXECUTED" && result.data) {
      ...
      // Embed constitution metadata into manifest
      (manifest as any).constitution_version = gk.constitution_version;
      (manifest as any).constitution_hash = gk.constitution_hash;
      (manifest as any).gatekeeper_mode = gk.mode;
      // Re-write manifest with constitution fields
      ...
      console.log(`Constitution: v${gk.constitution_version} | Gatekeeper: ${gk.mode}`);
  ...
  case "report": {
    ...
    // ---- Gatekeeper check (report is a side-effect: writes bundles/) ----
    const gk = runGatekeeper();
    printGatekeeperStatus(gk);
    ...
    if (result.status === "EXECUTED" && result.data) {
      // Embed constitution metadata
      (result.data as any).constitution_version = gk.constitution_version;
      (result.data as any).constitution_hash = gk.constitution_hash;
      (result.data as any).gatekeeper_mode = gk.mode;
  ...
  case "status": {
    ...
    console.log(`🏛️  Constitution: v${constitutionBoot.constitution_version} (hash: ${constitutionBoot.constitution_hash.slice(0, 16)}...)\n`);

    // Gatekeeper check
    const gk = runGatekeeper();
    console.log(`Gatekeeper Mode: ${gk.mode}`);
    if (gk.frozen_geo_expansion) console.log(`  🧊 Geo expansion: FROZEN`);
    if (gk.frozen_budget_increase) console.log(`  🧊 Budget increase: FROZEN`);
    if (gk.violations.length > 0) {
      for (const v of gk.violations) {
        const icon = v.severity === "block" ? "🛑" : "⚠️";
  ...
  case "metrics": {
    ...
    if (subcmd === "import" || flags.import) {
      ...
      // ---- Gatekeeper check (snapshot writing is a side-effect) ----
      const gk = runGatekeeper();
      printGatekeeperStatus(gk);
      ...
      if (result.status === "EXECUTED" && result.data) {
        ...
        // Embed constitution metadata into snapshot
        (s as any).constitution_version = gk.constitution_version;
        (s as any).constitution_hash = gk.constitution_hash;
        ...
        console.log(`  Constitution: v${gk.constitution_version}`);
      } else {
...
Constitution: v${constitutionBoot.constitution_version} (hash: ${constitutionBoot.constitution_hash.slice(0, 16)}...)

Commands:
  plan        --week <n|current>           Generate weekly plan (gatekeeper-gated)
  run         --operator <n> --command <cmd> [--intent <n>] [--type <t>] ...
  bundle      [--week <n>] [--intent <n>]  Assemble weekly bundle (gatekeeper-gated)
  report      --weekly [--week <n>]        Generate weekly report (gatekeeper-gated)
  status                                   Show orchestrator + constitution status
  approve     --operator <n> --pack <path> --item <id|all>
  validate    --bundle <path>
  metrics     --import [--date YYYY-MM-DD]    Import GSC CSVs (gatekeeper-gated)
              --latest                        Show latest snapshot
  experiment  --new [--week <n>] [--save]     Generate weekly experiments
  help

Environment:
  MBS_CONSTITUTION_PATH   Override constitution file path (default: config/constitution.json)
`);
}
